...
<div class="container-fluid">
    <h1 class="h3 mb-4 text-gray-800"><?= esc($pageTitle ?? 'Edit Subject Offering') ?></h1>
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form action="<?= site_url('admin/subject-offerings/update/' . $offering['id']) ?>" method="post">
                <?= csrf_field() ?>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select <?= (isset($validation) && $validation->hasError('subject_id')) ? 'is-invalid' : '' ?>"
                                ...
                                <?php foreach ($subjects as $subject) : ?>
                                    <option value="<?= $subject['id'] ?>" <?= old('subject_id', $offering['subject_id'] ?? '') == $subject['id'] ? 'selected' : '' ?>>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control <?= (isset($validation) && $validation->hasError('academic_year')) ? 'is-invalid' : '' ?>"
                                   ...
                                   value="<?= old('academic_year', $offering['academic_year'] ?? '') ?>" placeholder="e.g., 2023/2024" required>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select <?= (isset($validation) && $validation->hasError('semester')) ? 'is-invalid' : '' ?>"
                                ...
                                <?php foreach ($semesters as $key => $value) : ?>
                                    <option value="<?= $key ?>" <?= old('semester', $offering['semester'] ?? '') == $key ? 'selected' : '' ?>><?= esc($value) ?></option>
                                <?php endforeach; ?>
                            </select>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control <?= (isset($validation) && $validation->hasError('max_quota')) ? 'is-invalid' : '' ?>"
                                   id="max_quota" name="max_quota" value="<?= old('max_quota', $offering['max_quota'] ?? '') ?>" placeholder="Leave empty for no limit" min="0">
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control <?= (isset($validation) && $validation->hasError('description')) ? 'is-invalid' : '' ?>"
                              id="description" name="description" rows="3"><?= old('description', $offering['description'] ?? '') ?></textarea>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" value="<?= esc($offering['current_enrollment'] ?? '0') ?>" readonly>
                    <div class="form-text">This field is updated automatically based on student choices.</div>
                </div>

                <div class="mb-3 form-check">
                    <input type="checkbox" class="form-check-input" id="is_active" name="is_active" value="1" <?= old('is_active', $offering['is_active'] ?? false) ? 'checked' : '' ?>>
