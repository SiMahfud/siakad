...
<div class="container-fluid">
    <h1 class="h3 mb-4 text-gray-800"><?= esc($pageTitle ?? 'Edit Schedule') ?></h1>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control <?= (isset($validation) && $validation->hasError('academic_year')) ? 'is-invalid' : '' ?>"
                                   ...
                                   value="<?= old('academic_year', $schedule['academic_year'] ?? '') ?>" placeholder="e.g., 2023/2024" required>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select <?= (isset($validation) && $validation->hasError('semester')) ? 'is-invalid' : '' ?>"
                                ...
                                <?php foreach ($semesters as $key => $value) : ?>
                                    <option value="<?= $key ?>" <?= old('semester', $schedule['semester'] ?? '') == $key ? 'selected' : '' ?>><?= esc($value) ?></option>
                                <?php endforeach; ?>
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select <?= (isset($validation) && $validation->hasError('day_of_week')) ? 'is-invalid' : '' ?>"
                                ...
                                <?php foreach ($days as $key => $value) : ?>
                                    <option value="<?= $key ?>" <?= old('day_of_week', $schedule['day_of_week'] ?? '') == $key ? 'selected' : '' ?>><?= esc($value) ?></option>
                                <?php endforeach; ?>
                            </select>
...
<script>
    $(document).ready(function() {
        ...
        $('#day_of_week').select2({ theme: "bootstrap-5", minimumResultsForSearch: Infinity });
        $('#semester').select2({ theme: "bootstrap-5", minimumResultsForSearch: Infinity });
    });
</script>
<?= $this->endSection() ?>
