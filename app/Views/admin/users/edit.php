...
<div class="container-fluid">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800"><?= esc($title ?? 'Edit User') ?></h1>
    </div>

    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">User Details: <?= esc($user['full_name'] ?: $user['username']) ?></h6>
        </div>
        <div class="card-body">
            ...
            <form action="<?= site_url('admin/users/update/' . $user['id']) ?>" method="post">
                ...
                <div class="mb-3">
                    ...
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="is_active" id="is_active_true" value="1" <?= old('is_active', $user['is_active'] ?? '1') == '1' ? 'checked' : '' ?>>
                        <label class="form-check-label" for="is_active_true">Active</label>
                    </div>
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="is_active" id="is_active_false" value="0" <?= old('is_active', $user['is_active'] ?? '1') == '0' ? 'checked' : '' ?>>
