...
<?= $this->section('content') ?>
    ...
    <h1>Edit Subject: <?= esc($subject['subject_name'] ?? 'N/A') ?></h1>
    ...
    <form action="<?= site_url('admin/subjects/update/' . ($subject['id'] ?? '')) ?>" method="post">
        ...
        <div class="form-group">
            ...
            <select name="is_pilihan" id="is_pilihan">
                <option value="0" <?= old('is_pilihan', $subject['is_pilihan'] ?? '') == '0' ? 'selected' : '' ?>>Wajib (Core)</option>
                <option value="1" <?= old('is_pilihan', $subject['is_pilihan'] ?? '') == '1' ? 'selected' : '' ?>>Pilihan (Elective)</option>
            </select>
        </div>
