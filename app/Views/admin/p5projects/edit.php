...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <?= form_open('admin/p5projects/update/' . $project['id']) ?>
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="name">Project Name</label>
                            <input type="text" class="form-control <?= ($validation->hasError('name')) ? 'is-invalid' : '' ?>" id="name" name="name" value="<?= old('name', $project['name']) ?>" required>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control <?= ($validation->hasError('p5_theme_id')) ? 'is-invalid' : '' ?>" id="p5_theme_id" name="p5_theme_id" required>
                                ...
                                <?php foreach ($themes as $theme) : ?>
                                    <option value="<?= esc($theme['id']) ?>" <?= (old('p5_theme_id', $project['p5_theme_id']) == $theme['id']) ? 'selected' : '' ?>>
                                        <?= esc($theme['name']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control <?= ($validation->hasError('description')) ? 'is-invalid' : '' ?>" id="description" name="description" rows="3"><?= old('description', $project['description']) ?></textarea>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control <?= ($validation->hasError('start_date')) ? 'is-invalid' : '' ?>" id="start_date" name="start_date" value="<?= old('start_date', $project['start_date']) ?>">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control <?= ($validation->hasError('end_date')) ? 'is-invalid' : '' ?>" id="end_date" name="end_date" value="<?= old('end_date', $project['end_date']) ?>">
                ...
                <div class="form-group">
                    ...
                    <select multiple class="form-control select-sub-elements <?= ($validation->hasError('target_sub_elements')) ? 'is-invalid' : '' ?>" id="target_sub_elements" name="target_sub_elements[]">
                        <?php if (!empty($sub_elements)) : ?>
                            ...
                            $selected_ids = old('target_sub_elements', $current_project_sub_element_ids);
                            ?>
                                    ...
                                    <?php foreach ($elements_in_dim as $el_name => $sub_elements_in_el) : ?>
                                        <optgroup label="&nbsp;&nbsp;&nbsp;&nbsp;Elemen: <?= esc($el_name) ?>">
                                            <?php foreach ($sub_elements_in_el as $sub_element) : ?>
                                                <option value="<?= esc($sub_element['id']) ?>" <?= (is_array($selected_ids) && in_array($sub_element['id'], $selected_ids)) ? 'selected' : '' ?>>
